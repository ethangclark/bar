...
const exampleBase = 5789;

export const imageInjectorPrompt = (
  messages: Message[],
) => `Following is a series of messages between a student and an AI learning assistant. The AI learning assistant is only capable of interacting with the student via text -- hence you'll see some items in the initial system message that say e.g. "${imageHeaderWithOmissionDisclaimer(exampleBase)}".
...
<image>The image number of the omitted image, such as ${baseToId(exampleBase)}</image>
...
## Item 4
${fmtInfoImage(exampleBase, "The mitochondria are shown in purple, floating in the cytoplasm, in blue. They are kidney-shaped, and one of them has a cutaway that reveals its interior.")}
...
<image>${baseToId(exampleBase)}</image>
